...
```
      ...
      寄存器概念：vim 内置很多寄存器,比如 0-9,+-等，0 是默认(无名)寄存器,通常如删除复制操作(d, dd, s, S)等都会将内容存在 0 这个寄存器中，而系统 ctrl+c 的内容会存在+这个寄存器中（vscode 中显示*也存了，可通过底线命令:reg 查看。所以在没有相关 vim 配置的情况下，通过 p 操作（默认操作0寄存器）无法直接得到系统剪切版的内容，需要通过"+p这个操作才可以。具体配置可参考文章最后。
...
## 引申-附终端命令中常用的命令编辑快捷键(和 vim 有些相似)
