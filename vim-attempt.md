...
```
   ...
   寄存器：剪切版'+'，当前文件名'%',上次插入文本'.'
     寄存器：vim 内置很多寄存器,比如 0-9,+-等，0 是默认寄存器,通常如删除复制操作(d, dd, s, S)等都会将内容存在 0 这个
     寄存器中，而系统 ctrl-c 会存在+这个寄存器中（vscode 中显示\*也存了，可通过底线命令:reg 查看），所以在没有相关 vim 配置的情况
     通过 p 操作无法直接得到系统剪切版的内容，需要通过"+p 这个操作才可以。具体配置可参考文章最后。
```
